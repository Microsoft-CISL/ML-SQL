...
from Regression.Regression_BikeSharingDemand.regression_bike_sharing_sklearn.bike_sharing_fast_tree_sql_pipeline import BikeSharingFastTreeSQLPipeline
from python2sql.ml.utils import evaluate_regression_results
from python2sql.test.test_executor import TestExecutor


class BikeRegressionFastTreePipeline(object):
    ...
    def transform_and_fit(self, restore_save_trained_model=False):
        ...
        regressor_fast_tree = GradientBoostingRegressor(max_leaf_nodes=20, n_estimators=100, min_samples_leaf=10,
                                                        learning_rate=0.2, random_state=24)
        self.ml_pipeline.train_regressor(selected_dataset, self.y_train, selected_dataset.columns.values, regressor_fast_tree, save_restore_model=restore_save_trained_model)
    ...
    def predict_sql(self):
        ...
        table_name = "bike_sharing_FastTree"
        db_data = self.data.drop(self.drop_features[:-1], axis=1)  # i mantain in the data the id column
        ml_sql_pipepline = BikeSharingFastTreeSQLPipeline(db_data, table_name, self.ml_pipeline, self.predictions,
...
if __name__ == '__main__':
    ...
    bike_fast_tree_pipeline = BikeRegressionFastTreePipeline("BIKE_FAST_TREE", "hour_all_with_id.csv", "hour_train_with_id.csv", "hour_test_with_id.csv")
    test_executor = TestExecutor(bike_fast_tree_pipeline, ml_task="regression")
