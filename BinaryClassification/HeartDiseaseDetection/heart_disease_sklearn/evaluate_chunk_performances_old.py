...
from BinaryClassification.HeartDiseaseDetection.heart_disease_sklearn.heart_disease_binary_classification import HeartDiseaseBinaryClassificationPipeline


if __name__ == '__main__':
    hdbcp = HeartDiseaseBinaryClassificationPipeline("HeartAllWithId.csv", "HeartTrainingWithId.csv",
                                      "HeartTestWithId.csv")
    hdbcp.transform_and_fit()

    data_size = hdbcp.get_data().shape[0]
    full_dataset_file_path = hdbcp.get_dataset_file_path()
    data_header = hdbcp.get_data_header()
    class_attribute = hdbcp.get_class_attribute()
    ml_pipeline = hdbcp.get_ml_pipeline()
    OUTPUT_DIR = hdbcp.get_output_dir()
    ...
    times_evaluator = EvaluatePredictionTimes(full_dataset_file_path, data_size, data_header, class_attribute,
                                              chunk_sizes, output_methods, ml_pipeline, sep=';', header=False)
