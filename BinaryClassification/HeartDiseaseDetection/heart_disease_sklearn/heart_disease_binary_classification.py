...
from BinaryClassification.HeartDiseaseDetection.heart_disease_sklearn.heart_disease_sql_pipeline import HeartDiseaseSQLPipeline
from python2sql.ml.utils import evaluate_regression_results
from python2sql.test.test_executor import TestExecutor


class HeartDiseaseBinaryClassificationPipeline(object):
    ...
    def transform_and_fit(self, restore_save_trained_model=False):
        ...
        # COMPLETE DATASET
        self.data = pd.read_csv(self.dataset_file_path, header=None, names=header, sep=';')
        ...
        # TRAIN
        self.train = pd.read_csv(self.train_dataset_file_path, header=None, names=header, sep=';')
        self.y_train = self.train[self.class_attribute]
        self.X_train = self.train.drop([self.class_attribute], axis=1)

        # TEST
        self.test = pd.read_csv(self.test_dataset_file_path, header=None, names=header, sep=';')
        ...
        self.ml_pipeline.train_classifier(selected_dataset, self.y_train, features, classifier, save_restore_model=restore_save_trained_model)
    ...
    def predict_sql(self):
        ...
        table_name = "heart_disease_detection_with_score"
        ml_sql_pipepline = HeartDiseaseSQLPipeline(self.data, table_name, self.ml_pipeline, self.predictions, self.probabilities, self.scores)
        # ml_sql_pipepline = HeartDiseaseSQLPipeline(self.test, table_name, self.ml_pipeline, self.predictions, self.probabilities, self.scores)
...
if __name__ == '__main__':
    ...
    heart_disease_pipeline = HeartDiseaseBinaryClassificationPipeline("HEART_DISEASE", "HeartAllWithId.csv", "HeartTrainingWithId.csv",
                                      "HeartTestWithId.csv")
    test_executor = TestExecutor(heart_disease_pipeline, sep=';', header=False)
