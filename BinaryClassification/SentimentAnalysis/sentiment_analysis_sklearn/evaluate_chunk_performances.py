...
from BinaryClassification.SentimentAnalysis.sentiment_analysis_sklearn.sentiment_analysis_binary_classification import *
...
if __name__ == '__main__':
    sap = SentimentAnalysisPipeline("sentiment_data.csv", "sentiment_training_data.csv", "sentiment_test_data.csv")
    sap.transform_and_fit()

    data_size = sap.get_data().shape[0]
    full_dataset_file_path = sap.get_dataset_file_path()
    data_header = sap.get_data_header()
    class_attribute = sap.get_class_attribute()
    ml_pipeline = sap.get_ml_pipeline()
    OUTPUT_DIR = sap.get_output_dir()
