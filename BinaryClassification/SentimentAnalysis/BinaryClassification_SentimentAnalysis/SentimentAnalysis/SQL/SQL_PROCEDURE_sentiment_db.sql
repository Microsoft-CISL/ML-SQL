delimiter //
set names utf8;
drop procedure if exists MLtoSQL.sentiment_db;
CREATE procedure MLtoSQL.sentiment_db(num INT, chuncksize INT)
wholeblock:BEGIN
SET @id = 1;
WHILE @id <= num DO
insert into sentiment_analysis_detection_with_score_output 
 select * from (SELECT id, (SUM( F1.count * weights_sentiment_analysis.weight) +-2.540378) AS weight FROM
