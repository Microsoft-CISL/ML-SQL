...
SET @chuncksize =100;
 ...
 #select * from (SELECT id, (SUM( F1.count * weights_sentiment_analysis.weight) +-2.561048) AS weight FROM
#(
...
#as F1 
;
#INNER JOIN weights_sentiment_analysis ON (weights_sentiment_analysis.label = F1.feature) group by id
# ) AS F;