...
from BinaryClassification.CreditCardFraudDetection.credit_card_scikit_learn.credit_card_binary_classification import CreditCardClassifcationPipeline

if __name__ == '__main__':
    cccp = CreditCardClassifcationPipeline("creditcard.csv", "credit_train_data.csv", "credit_test_data.csv")
    cccp.transform_and_fit()

    data_size = cccp.get_data().shape[0]
    full_dataset_file_path = cccp.get_dataset_file_path()
    data_header = cccp.get_data_header()
    class_attribute = cccp.get_class_attribute()
    ml_pipeline = cccp.get_ml_pipeline()
    OUTPUT_DIR = cccp.get_output_dir()
    ...
    times_evaluator = EvaluatePredictionTimes(full_dataset_file_path, data_size, data_header, class_attribute,
                                              chunk_sizes, output_methods, ml_pipeline)
    ...
    full_dataset_file_path = os.path.join(DATA_DIR, "dataset", "creditcard.csv")
    ...
    # FEATURE REMOVAL
    drop_features = ["Time", "id"]
    selected_dataset = ml_pipeline.drop_features_from_data(X_train, drop_features)

    # TRAIN SET TRANSFORMATION
    transformer = StandardScaler()
    transformed_dataset = ml_pipeline.apply_transformation(selected_dataset, transformer)
